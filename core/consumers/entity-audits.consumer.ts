...
import { EntityAuditsConsumerEnum, QueueNamesEnum } from '../common/enums/consumers';
...
export interface RecordEntityAuditJobData extends Omit<RecordEntityAuditDto, 'userOrBusinessReq'> {
...
export class EntityAuditsConsumer extends WorkerHost {
     ...
     */
    private async processRecordAudit(job: Job<RecordEntityAuditJobData>): Promise<void> {
        const { entityName, entityId, operation, oldValues, newValues, userOrBusinessReq } = job.data;
        ...
        const hasCreator = userOrBusinessReq.businessId != null || userOrBusinessReq.userId != null;
        if (!hasCreator) {
            LogWarn(this.log, `Missing businessId or userId in job ${job.id}`, this.processRecordAudit.name);
            return;
        }
