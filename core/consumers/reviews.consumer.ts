...
interface CalculateAverageJobData {
  idProduct: number;
  user: IUserReq;
}
...
export class ReviewsConsumer extends WorkerHost {
   ...
   */
  private async processCalculateAverage(
    job: Job<CalculateAverageJobData>,
  ): Promise<void> {
    ...
    if (!idProduct) {
      LogWarn(
        ...
        this.processCalculateAverage.name,
      );
      return;
    }
    const productReviews =
      await this.productReviewsGettersService.findAllByProduct(idProduct);
    ...
    await this.productsSettersService.updateRatingAverage(
      product,
      ratingAverage,
      user,
    );
