...
export class CatalogsConsumer extends WorkerHost {
   ...
   */
  private async processUpdateProductsCount(
    job: Job<UpdateProductsCountJobData>,
  ): Promise<void> {
    const { idCatalog, action, businessReq } = job.data;
    if (!idCatalog || !action) {
      LogWarn(
        this.log,
        `Missing idCatalog or action in job ${job.id}`,
        this.processUpdateProductsCount.name,
      );
      return;
    }
    const catalog = await this.catalogsGettersService.findOne(idCatalog);
    await this.catalogsSettersService.updateProductsCount(
      catalog,
      action,
      businessReq,
    );
