...
export class AdminStatisticsGettersService {
  ...
  async getUserStats(timePeriod?: ITimePeriodFilter): Promise<IAdminUserStats> {
    const totalUsers =
      await this.usersGettersService.getNonDeletedUsersCountForAdminStatistics();
    const usersByStatus =
      await this.usersGettersService.getUsersGroupedByStatusForAdminStatistics();
    let newUsersInPeriod: IAdminTimeSeriesStats | undefined;
    if (timePeriod?.startDate && timePeriod?.endDate) {
      newUsersInPeriod =
        await this.usersGettersService.getNewUsersStatsForAdminStatistics(
          timePeriod,
        );
    }
  ...
  ): Promise<IAdminBusinessStats> {
    const totalBusinesses =
      await this.businessesGettersService.getNonDeletedBusinessesCountForAdminStatistics();
    const onlineBusinessesCount =
      await this.businessesGettersService.getOnlineNonDeletedBusinessesCountForAdminStatistics();
    const businessesByStatus =
      await this.businessesGettersService.getBusinessesGroupedByStatusForAdminStatistics();
    let newBusinessesInPeriod: IAdminTimeSeriesStats | undefined;
    if (timePeriod?.startDate && timePeriod?.endDate) {
      newBusinessesInPeriod =
        await this.businessesGettersService.getNewBusinessesStatsForAdminStatistics(
          timePeriod,
        );
    }
    return {
  ...
  async getPlatformEngagementStats(
    ...
    const businessVisits =
      await this.businessVisitsGettersService.getGlobalVisitStatsForAdminStatistics(
        timePeriod,
      );
    const productVisits =
      await this.productVisitsGettersService.getGlobalVisitStatsForAdminStatistics(
        timePeriod,
      );
    const catalogVisits =
      await this.catalogVisitsGettersService.getGlobalVisitStatsForAdminStatistics(
        timePeriod,
      );
  ...
  async getCatalogGlobalStats(): Promise<IAdminCatalogGlobalStats> {
    const totalProducts =
      await this.productsGettersService.getNonDeletedProductsCountForAdminStatistics();
    const totalSkus =
      await this.productSkusGettersService.getActiveSkusForNonDeletedProductsCountForAdminStatistics();
    const productsWithoutStock =
      await this.productsGettersService.getGlobalProductsWithoutStockCountForAdminStatistics();
  ...
  async getDiscountGlobalStats(
    ...
    const discounts =
      await this.discountsGettersService.findDiscountDateRangesForAdminStatistics();
    const discountsByStatus = this.bucketDiscountsByLifecycle(discounts);
    const discountsByType =
      await this.discountsGettersService.getGlobalDiscountsByTypeForAdminStatistics();
    const expiringSoonCount =
      await this.discountsGettersService.getGlobalExpiringSoonDiscountCountForAdminStatistics(
        days,
      );
