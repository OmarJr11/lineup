...
export class DiscountProductsService extends BasicService<DiscountProduct> {
    ...
    async findByProductIdWithDiscount(idProduct: number): Promise<DiscountProduct> {
        return await this.discountProductsGettersService.findByProductIdWithDiscount(idProduct);
    }
    ...
    async findAllByDiscountId(idDiscount: number): Promise<DiscountProduct[]> {
        return await this.discountProductsGettersService.findAllByDiscountId(idDiscount);
    }
    ...
    async upsert(
        ...
        if (existing) {
            await this.discountProductsSettersService.updateDiscount(existing, idDiscount, businessReq);
            return await this.discountProductsGettersService.findByProductId(idProduct);
        }
    ...
    async removeByDiscountId(idDiscount: number, businessReq: IBusinessReq) {
        const discountProducts = await this.discountProductsGettersService.findAllByDiscountId(idDiscount);
