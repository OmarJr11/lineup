...
export class ProductCollectionsService {
    ...
    private async buildCollectionFromVisitedTags(
        ...
        const tagIds = await this.productVisitsGettersService.getTagIdsFromVisitedProducts(idUser);
        if (tagIds.length === 0) {
            return { id: 'visited-tags', title: 'Basado en lo que visitaste', products: [] };
        }
        const productIds = await this.productsGettersService.findProductIdsByTagIds(tagIds, DEFAULT_COLLECTION_LIMIT);
        const products = await this.productsGettersService.findManyWithRelations(productIds);
        return {
            id: 'visited-tags',
     ...
     */
    private async buildCollectionFromLikedTags(idUser: number): Promise<IProductCollection> {
        const tagIds = await this.productReactionsGettersService.getTagIdsFromLikedProducts(idUser);
        if (tagIds.length === 0) {
     ...
     */
    private async buildCollectionByUserLocation(idUser: number): Promise<IProductCollection> {
        ...
        const productIds = await this.getProductIdsByLocationRandom(stateName, DEFAULT_COLLECTION_LIMIT);
        const products = await this.productsGettersService.findManyWithRelations(productIds);
        return {
            id: 'location',
            title: 'Cerca de ti',
     ...
     */
    private async buildCollectionFromSearchHistory(idUser: number): Promise<IProductCollection> {
        const searchTerms = await this.userSearchesService.getRecentSearchTerms(idUser, 3);
        ...
        for (const term of searchTerms) {
            ...
            const productItems = result.items.filter((i) => i.__typename === 'ProductSchema');
        ...
        const productIds = this.shuffleArray(allProductIds).slice(0, DEFAULT_COLLECTION_LIMIT);
        const products = await this.productsGettersService.findManyWithRelations(productIds);
        return {
            id: 'searches',
     ...
     */
    private async getProductIdsByLocationRandom(stateName: string, limit: number): Promise<number[]> {
        return await this.productSearchIndexGettersService.getProductIdsByLocation(stateName, limit);
    }
    ...
    private async getTopRatedProductIds(limit: number): Promise<number[]> {
        return await this.productSearchIndexGettersService.getTopRatedProductIds(limit);
    }
    ...
    private async getMostVisitedProductIds(limit: number): Promise<number[]> {
        return await this.productSearchIndexGettersService.getMostVisitedProductIds(limit);
    }
     ...
     */
    private orderByIds<T extends { id?: number | string }>(items: T[], ids: number[]): T[] {
        const byId = new Map<number, T>(items.map((item) => [Number(item.id), item]));
