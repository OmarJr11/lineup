...
export class DiscountsService extends BasicService<Discount> {
    ...
    async update(
        ...
        const discount = await this.discountsGettersService
            .findOneAndVerifyOwnership(data.id, businessReq.businessId);
        return await this.discountsSettersService.updateDiscount(discount, data, businessReq);
    }
    ...
    async remove(id: number, businessReq: IBusinessReq) {
        const discount = await this.discountsGettersService
            .findOneAndVerifyOwnership(id, businessReq.businessId);
    ...
    async findOne(id: number, businessReq: IBusinessReq): Promise<Discount> {
        return await this.discountsGettersService
            .findOneAndVerifyOwnership(id, businessReq.businessId);
    }
    ...
    async findOneByBusiness(id: number, idBusiness: number): Promise<Discount> {
        return await this.discountsGettersService
            .findOneAndVerifyOwnership(id, idBusiness);
    }
    ...
    async findAllMyDiscounts(businessReq: IBusinessReq): Promise<Discount[]> {
        return await this.discountsGettersService
            .findAllByBusiness(businessReq.businessId);
    }
    ...
    async findAllMyDiscountsByScope(
        ...
        switch (input.scope) {
            ...
            case DiscountScopeEnum.PRODUCT:
                return await this.discountsGettersService
                    .findAllByScopePaginated(input.scope, idBusiness, pagination);
            default:
                LogError(
                    this.logger,
    ...
    async findActiveDiscountByProduct(
        ...
        return await this.discountsGettersService.findActiveDiscountByProduct(idProduct);
    }
    ...
    private async validateScopeOwnership(
        ...
        switch (data.scope) {
            case DiscountScopeEnum.BUSINESS:
                ...
                break;
            case DiscountScopeEnum.CATALOG:
                await this.catalogsGettersService
                    .checkIfExistsByIdAndBusinessId(data.idCatalog, idBusiness);
                break;
            case DiscountScopeEnum.PRODUCT:
                await this.productsGettersService.findOneByBusinessId(data.idProduct, idBusiness);
                break;
            default:
                LogError(this.logger, this.rCreate.cantAssignDiscount.message, this.create.name);
                throw new ForbiddenException(this.rCreate.cantAssignDiscount);
        }
    }
    ...
    private async resolveProductIds(
        ...
        switch (data.scope) {
            case DiscountScopeEnum.BUSINESS:
                return await this.productsGettersService.findProductIdsByBusiness(idBusiness);
                break;
            case DiscountScopeEnum.CATALOG:
                return await this.productsGettersService.findProductIdsByCatalog(data.idCatalog);
                break;
            case DiscountScopeEnum.PRODUCT:
