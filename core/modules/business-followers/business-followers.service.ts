...
export class BusinessFollowersService extends BasicService<BusinessFollower> {
  ...
  async followBusiness(
    ...
    const data: ICreateBusinessFollower = {
      idBusiness,
      idCreationUser: userReq.userId,
    };
    const follower = await this.businessFollowersSettersService.create(
      data,
      userReq,
    );
    void this.incrementBusinessLikes(business, userReq);
  ...
  async unfollowBusiness(
    ...
    await this.businessFollowersSettersService.remove(
      existingFollower,
      userReq,
    );
    void this.decrementBusinessLikes(business, userReq);
    await this.searchDataQueue.add(
      SearchDataConsumerEnum.SearchDataBusinessFollowRecord,
