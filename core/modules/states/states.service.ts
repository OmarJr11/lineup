import { ConflictException, Inject, Injectable, Logger, NotAcceptableException } from '@nestjs/common';
...
export class StatesService extends BasicService<State> {
     ...
     */
    private async validateUniqueFieldsOnUpdate(data: UpdateStateInput, userReq: IUserReq) {
        if (
            data.name &&
            (await this.statesGettersService.existsByName(data.name, data.id))
        ) {
            LogError(this.logger, statesResponses.update.nameAlreadyExists, this.update.name, userReq);
            throw new NotAcceptableException(statesResponses.update.nameAlreadyExists);
        }
        if (
            data.code &&
            (await this.statesGettersService.existsByCode(data.code, data.id))
        ) {
            LogError(this.logger, statesResponses.update.codeAlreadyExists, this.update.name, userReq);
            throw new NotAcceptableException(statesResponses.update.codeAlreadyExists);
        }
        if (
            data.capital &&
            (await this.statesGettersService.existsByCapital(data.capital, data.id))
        ) {
            LogError(this.logger, statesResponses.update.capitalAlreadyExists, this.update.name, userReq);
            throw new NotAcceptableException(statesResponses.update.capitalAlreadyExists);
        }
    }
}
