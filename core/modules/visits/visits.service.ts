...
export class VisitsService {
   ...
   */
  @Transactional()
  async recordVisit(input: IRecordVisitInput, user: IUserReq | null) {
    switch (input.type) {
      case VisitTypeEnum.BUSINESS:
        await this.businessVisitsSettersService.create(
          { idBusiness: input.id },
          user,
        );
        break;
      case VisitTypeEnum.PRODUCT:
        await this.productVisitsSettersService.create(
          { idProduct: input.id },
          user,
        );
        break;
      case VisitTypeEnum.CATALOG:
        await this.catalogVisitsSettersService.create(
          { idCatalog: input.id },
          user,
        );
        break;
