...
export class CatalogsService extends BasicService<Catalog> {
  ...
  async create(
    ...
    const tittleFormatted = this.catalogsSettersService.generatePathFromTitle(
      data.title,
    );
    const path =
      data.path ??
      (await this.catalogsGettersService.checkCatalogPathExists(
        tittleFormatted,
      ));
    data.path = path;
    const catalog = await this.catalogsSettersService.create(data, businessReq);
    await this.searchDataQueue.add(SearchDataConsumerEnum.SearchDataCatalog, {
  ...
  async findAllMyCatalogs(
    ...
    return await this.catalogsGettersService.findAllMyCatalogs(
      query,
      businessReq,
    );
  }
  ...
  async findAllByBusinessId(
    ...
    return await this.catalogsGettersService.findAllByBusinessId(
      idBusiness,
      query,
    );
  }
  ...
  async update(
    ...
    if (data.title && data.title !== catalog.title) {
      ...
      const path = await this.catalogsGettersService.checkCatalogPathExists(
        tittleFormatted,
        catalog.id,
      );
      data.path = path;
