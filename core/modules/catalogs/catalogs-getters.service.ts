...
export class CatalogsGettersService extends BasicService<Catalog> {
  ...
  async findAllMyCatalogs(
    ...
    return await this.getQueryRelations(this.createQueryBuilder('c'))
      .where('c.status <> :status', { status: StatusEnum.DELETED })
      .andWhere('c.idCreationBusiness = :idCreationBusiness', {
        idCreationBusiness: business.businessId,
      })
      .limit(limit)
      .offset(skip)
      .orderBy(`c.${orderBy}`, order)
  ...
  async checkCatalogPathExists(
    ...
    do {
      const indexStr =
        index < 10 ? index.toString().padStart(2, '0') : index.toString();
      ...
      index++;
    } while (true); // eslint-disable-line no-constant-condition -- unique path until break
  ...
  private getQueryRelations(
    ...
    return queryBuilder
      ...
      .leftJoinAndSelect(
        ...
        'productFiles',
        'productFiles.status <> :productFileStatus',
        { productFileStatus: StatusEnum.DELETED },
      )
      .leftJoinAndSelect('productFiles.file', 'productFilesFile')
      .leftJoinAndSelect(
        'products.variations',
        'variations',
        'variations.status <> :variationStatus',
        { variationStatus: StatusEnum.DELETED },
      )
