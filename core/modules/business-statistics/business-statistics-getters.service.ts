...
export class BusinessStatisticsGettersService {
    ...
    async getBusinessVisits(
        ...
        if (StatisticsQueryHelper.shouldGroupByTime(timePeriod)) {
            const data = await this.businessVisitsGettersService
                .getTimeSeriesByBusiness(idBusiness, timePeriod!);
            const total = data.reduce((sum, d) => sum + d.value, 0);
            return { total, data };
        }
        const total = await this.businessVisitsGettersService
            .getCountByBusiness(idBusiness, timePeriod);
        return { total };
    }
    ...
    async getBusinessVisitsByAuthType(
        ...
        const { anonymous, identified } = await this.businessVisitsGettersService
            .getCountByAuthType(idBusiness, timePeriod);
    ...
    async getNewFollowers(
        ...
        if (StatisticsQueryHelper.shouldGroupByTime(timePeriod)) {
            const data = await this.businessFollowersGettersService
                .getTimeSeriesForStatistics(idBusiness, timePeriod!);
            const total = data.reduce((sum, d) => sum + d.value, 0);
            return { total, data };
        }
        const total = await this.businessFollowersGettersService
            .getCountForStatistics(idBusiness, timePeriod);
        return { total };
    }
    ...
    async getTopProductsByVisits(
        ...
        const visitData = await this.productVisitsGettersService.getTopProductsByVisits(
            idBusiness,
            timePeriod,
            limit,
        );
    ...
    async getTopProductsByRating(
        ...
        return await this.productsGettersService
            .getTopByRatingForStatistics(idBusiness, limit);
    }
    ...
    async getProductsWithoutVisitsCount(idBusiness: number): Promise<number> {
        return await this.productsGettersService.getWithoutVisitsCountForStatistics(idBusiness);
    }
    ...
    async getProductsWithoutRatingsCount(idBusiness: number): Promise<number> {
        return await this.productsGettersService.getWithoutRatingsCountForStatistics(idBusiness);
    }
    ...
    async getVisitToLikeRatio(
        ...
        const products = await this.productsGettersService.getProductIdsAndLikesForStatistics(
            idBusiness,
        );
        ...
        const totalVisits = await this.productVisitsGettersService.getVisitCountByProductIds(
            productIds,
            timePeriod,
        );
        const totalLikes = await this.productsGettersService.getTotalLikesByProductIds(productIds);
     ...
     */
    async getProductsPerCatalog(idBusiness: number): Promise<IFrequencyDataPoint[]> {
        return this.catalogsGettersService.getProductsPerCatalogForStatistics(idBusiness);
    }
    ...
    async getCatalogVisitsOverTime(
        ...
        if (StatisticsQueryHelper.shouldGroupByTime(timePeriod)) {
            const data = await this.catalogVisitsGettersService.getTimeSeriesByBusiness(
                idBusiness,
                timePeriod!,
            );
            const total = data.reduce((sum, d) => sum + d.value, 0);
    ...
    async getCatalogStats(
        ...
        const [topByVisits, productsPerCatalog, catalogVisitsOverTime] = await Promise.all([
     ...
     */
    async getDiscountsByStatus(idBusiness: number): Promise<IFrequencyDataPoint[]> {
    ...
    async getProductsWithoutStockCount(idBusiness: number): Promise<number> {
        return this.productsGettersService.getWithoutStockCountForStatistics(idBusiness);
    }
