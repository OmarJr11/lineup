...
export class ProductSkusService extends BasicService<ProductSku> {
  ...
  async findOneByBusinessId(
    ...
    return await this.productSkusGettersService.findOneByBusinessId(
      id,
      idBusiness,
    );
  }
  ...
  async findAllByProductAndBusiness(
    ...
    return await this.productSkusGettersService.findAllByProductAndBusiness(
      idProduct,
      idBusiness,
    );
  }
  ...
  async updateAllSkusByProduct(
      ...
      if (item.quantity !== undefined) {
        ...
        if (quantityDelta !== 0) {
          await this.adjustStock(
            {
              idProductSku: item.id,
              quantityDelta,
              notes: 'Update from product SKUs edit',
            },
            businessReq,
          );
        }
    ...
    return Promise.all(
      updatedIds.map((id) =>
        this.productSkusGettersService.findOneWithRelations(id),
      ),
    );
  }
