...
export class ProductReactionsService extends BasicService<ProductReaction> {
  ...
  async likeProduct(
    ...
    const reaction = await this.productReactionsSettersService.create(
      data,
      userReq,
    );
    void this.incrementProductLikes(product, userReq);
   ...
   */
  @Transactional()
  async unlikeProduct(idProduct: number, userReq: IUserReq): Promise<boolean> {
    const product = await this.productsGettersService.findOne(idProduct);
    const existingReaction =
      await this.productReactionsGettersService.findOneByProductAndUser(
        idProduct,
    ...
    void this.decrementProductLikes(product, userReq);
    return true;
  }
