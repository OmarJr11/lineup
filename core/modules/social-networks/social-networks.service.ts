...
export class SocialNetworksService extends BasicService<SocialNetwork> {
  ...
  async create(
    ...
    const socialNetwork = await this.socialNetworksSettersService.create(
      data,
      user,
    );
    return await this.socialNetworksGettersService.findById(socialNetwork.id);
  }
  ...
  async update(
    ...
    const socialNetwork = await this.socialNetworksGettersService.findById(
      data.id,
    );
    await this.socialNetworksSettersService.update(data, socialNetwork, user);
    return await this.socialNetworksGettersService.findById(socialNetwork.id);
  }
  ...
  async remove(id: number, user: IUserReq): Promise<boolean> {
    const socialNetwork = await this.socialNetworksGettersService.findById(id);
    await this.socialNetworksSettersService.remove(socialNetwork, user);
    return true;
  }
}
