...
export class ProductRatingsGettersService extends BasicService<ProductRating> {
  ...
  async findAllByUserPaginated(
    ...
    try {
      ...
      const subQuery = this.createQueryBuilder('sub')
        ...
        .limit(limit)
        .offset(skip);
      let queryBuilder: SelectQueryBuilder<ProductRating> =
      ...
      return await queryBuilder
        ...
        .where(`pr.id IN (${subQuery.getQuery()})`)
        .andWhere('product.status <> :statusProduct', {
          statusProduct: StatusEnum.DELETED,
        })
   ...
   */
  private formatProductRatings(
    productRatings: ProductRating[],
  ): ProductRating[] {
    return productRatings.map((productRating) =>
      this.formatProductRating(productRating),
    );
  }
  ...
  private formatProductRating(productRating: ProductRating): ProductRating {
    if (
      ...
      productRating.product?.productFiles.length > 0
    ) {
      const productFiles = productRating.product.productFiles.filter(
        (productFile) => productFile.status !== StatusEnum.DELETED,
      );
