...
export class SearchIndexService {
  ...
  buildProductRawText(product: Product): string {
    ...
    if (product.skus?.length) {
      const skuText = product.skus
        .map((s) =>
          [s.skuCode, ...Object.values(s.variationOptions ?? {})].join(' '),
        )
        .join(' ');
      parts.push(skuText);
    }
  ...
  buildCatalogRawText(catalog: Catalog): string {
    ...
    if (catalog.products?.length) {
      const productTitles = catalog.products
        .map((p) => p.title ?? '')
        .join(' ');
  ...
  async upsertProductSearchIndex(product: Product): Promise<void> {
    ...
    const enhancedText = await this.enhanceSearchText(
      rawText,
      buildProductSearchEnhancementPrompt,
    );
    ...
    const skuPrices =
      product.skus?.map((s) => s.price).filter((p): p is number => p != null) ??
      [];
    const price =
      skuPrices.length > 0 ? Math.min(...skuPrices.map(Number)) : null;
    const locationsText =
   ...
   */
  async updateProductRatingAverage(
    idProduct: number,
