...
export class SearchService {
  ...
  async search(
    ...
    const effectiveTarget = hasProductFilters
      ? SearchTargetEnum.PRODUCTS
      : target;

    if (!searchTerm) {
      return this.fetchRandomItems(
        effectiveTarget,
        limit,
        offset,
        page,
    ...
    try {
      const rows = await this.executeSearchQuery(
        effectiveTarget,
        searchTerm,
        limit,
        offset,
  ...
  async getFeaturedBusinesses(
    ...
    try {
      ...
      const businesses =
        ids.length > 0
          ? await this.fetchBusinessesByIds(ids)
  ...
  async getFeaturedCatalogs(
    ...
    try {
      ...
      const catalogs =
        ids.length > 0
          ? await this.fetchCatalogsByIds(ids)
          : new Map<number, Catalog>();
      const items = ids
        .map((id) => catalogs.get(id))
  ...
  private async fetchRandomItems(
    ...
    try {
      const rows = await this.executeRandomQuery(
        target,
        limit,
        offset,
        productFilters,
      ...
      this.logger.warn(
        `Random fetch failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      return { items: [], total: 0, page, limit };
    }
  }
   ...
   */
  private hasActiveProductFilters(
   ...
   */
  private buildProductFilterConditions(
    productFilters?: ProductSearchFiltersInput,
  ): {
    conditions: string[];
    params: unknown[];
    ...
    if (productFilters.minPrice != null) {
      ...
      conditions.push(
        `psi.price IS NOT NULL AND (psi.price)::numeric >= ($${idx++})::numeric`,
      );
    }
  ...
  private async executeRandomQuery(
    ...
    if (
      ...
      parts.push(
        `(SELECT id, 'business' AS type, 0::float AS rank FROM businesses WHERE status <> 'deleted')`,
      );
    }
    if (
      target === SearchTargetEnum.ALL ||
      target === SearchTargetEnum.CATALOGS
    ) {
    ...
    if (
      ...
      const { conditions, params } =
        this.buildProductFilterConditions(productFilters);
      const filterClause =
  ...
  private async executeRandomCountQuery(
    ...
    if (
      ...
      parts.push(
        `(SELECT COUNT(*)::int FROM businesses WHERE status <> 'deleted')`,
      );
    }
    if (
      target === SearchTargetEnum.ALL ||
  ...
  private async executeSearchQuery(
    ...
    const fetchLimit =
      target === SearchTargetEnum.ALL ? limit + offset + 200 : limit + offset;
   ...
   */
  private async fetchEntitiesFromRows(
    ...
    const [businesses, catalogs, products] = await Promise.all([
      businessIds.length > 0
        ? this.fetchBusinessesByIds(businessIds)
        : Promise.resolve(new Map<number, Business>()),
      catalogIds.length > 0
        ? this.fetchCatalogsByIds(catalogIds)
        : Promise.resolve(new Map<number, Catalog>()),
      productIds.length > 0
        ? this.fetchProductsByIds(productIds)
        : Promise.resolve(new Map<number, Product>()),
    ]);
   ...
   */
  private async fetchCatalogsByIds(
    ids: number[],
  ): Promise<Map<number, Catalog>> {
