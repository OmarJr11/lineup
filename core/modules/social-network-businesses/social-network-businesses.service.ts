...
export class SocialNetworkBusinessesService extends BasicService<SocialNetworkBusiness> {
  ...
  async create(
    ...
    const socialNetworkBusiness = await this.settersService.create(
      data,
      businessReq,
    );
  ...
  private validateContactMatchesSocialNetwork(
    ...
    if (
      hasUrl &&
      SOCIAL_NETWORKS_REQUIRING_PHONE.includes(socialNetwork.code)
    ) {
      LogError(
        this.logger,
        this.rCreate.contactMismatch.message,
        this.validateContactMatchesSocialNetwork.name,
      );
      throw new NotAcceptableException(this.rCreate.contactMismatch);
    }
    if (
      hasPhone &&
      SOCIAL_NETWORKS_REQUIRING_URL.includes(socialNetwork.code)
    ) {
      LogError(
        this.logger,
        this.rCreate.contactMismatch.message,
  ...
  private async checkIfSocialNetworkBusinessExists(
    ...
    const socialNetwork =
      await this.socialNetworksGettersService.findById(idSocialNetwork);
    this.validateContactMatchesSocialNetwork(contact, socialNetwork);
    const exists =
      await this.gettersService.checkIfExistsByIdBusinessAndSocialNetwork(
        business.id,
        socialNetwork.id,
      );
    if (exists) {
