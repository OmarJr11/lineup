...
export class DiscountsCronService {
  ...
  async activatePendingDiscounts(): Promise<void> {
    const pendingDiscounts =
      await this.discountsGettersService.findAllPendingWithStartDateReached();
    if (pendingDiscounts.length > 0) {
      const ids = pendingDiscounts.map((discount) => discount.id);
      await this.discountsQueue.add(DiscountsConsumerEnum.ActivateDiscount, {
        ids,
      });
    }
  }
  ...
  async removeExpiredDiscounts(): Promise<void> {
    const expiredDiscounts =
      await this.discountsGettersService.findAllActiveWithEndDatePassed();
