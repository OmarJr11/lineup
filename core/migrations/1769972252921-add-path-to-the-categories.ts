...
export class AddPathToTheCategories1769972252921 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "catalogs" ADD "path" character varying(255)`,
    );
    ...
    for (const catalog of catalogs) {
      ...
      const uniquePath = await this.checkCatalogPathExists(
        queryRunner,
        basePath,
        catalog.id,
      );
    ...
    await queryRunner.query(
      `ALTER TABLE "catalogs" ALTER COLUMN "path" SET NOT NULL`,
    );

    // 5) Add unique constraint
    await queryRunner.query(
      `ALTER TABLE "catalogs" ADD CONSTRAINT "UQ_96264735a474e45d8c02524f768" UNIQUE ("path")`,
    );

    // 6) Update title column length
    // Create temporary column to store current title values
    await queryRunner.query(
      `ALTER TABLE "catalogs" ADD "titleCopy" character varying(255)`,
    );
    await queryRunner.query(`UPDATE "catalogs" SET "titleCopy" = "title"`);

    // Handle NULL values in titleCopy (set a default value if needed)
    await queryRunner.query(
      `UPDATE "catalogs" SET "titleCopy" = 'Untitled Catalog' WHERE "titleCopy" IS NULL`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "catalogs" ADD "title" character varying(150)`,
    );

    // Copy values from titleCopy to title (truncate if necessary)
    await queryRunner.query(
      `UPDATE "catalogs" SET "title" = LEFT("titleCopy", 150)`,
    );

    // Handle any remaining NULL values (shouldn't happen, but just in case)
    await queryRunner.query(
      `UPDATE "catalogs" SET "title" = 'Untitled Catalog' WHERE "title" IS NULL`,
    );

    // Now make title NOT NULL
    await queryRunner.query(
      `ALTER TABLE "catalogs" ALTER COLUMN "title" SET NOT NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE "catalogs" DROP COLUMN "title"`);
    await queryRunner.query(
      `ALTER TABLE "catalogs" ADD "title" character varying(255) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "catalogs" DROP CONSTRAINT "UQ_96264735a474e45d8c02524f768"`,
    );
