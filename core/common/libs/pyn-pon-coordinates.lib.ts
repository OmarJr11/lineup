 ...
 * @param {IAddressComponent[]} data - address components
 * @param {string} type - type to search
 * @returns {IAddressComponent} - address component found
 */
...
export function convertCoordinateToString(
  coordinate: ICoordinate | string,
): string {
 ...
 */
function getGradDifferenceByPoints(
  coord1: ICoordinate,
  coord2: ICoordinate,
): number {
  const sum =
