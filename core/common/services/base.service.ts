...
import type { IUserOrBusinessReq } from '../interfaces';
...
export class BasicService<Entity extends ObjectLiteral> {
   ...
   */
  async findByIdWithoutRelationsOrFail(
    id: number,
    error: any,
  ): Promise<Entity> {
  ...
  async saveAndGetRelations(
    ...
    const savedEntity = await this.repository.save(
      this.cleanDataBeforeInsert(data),
      {
        data: user,
      },
    );
   ...
   * @returns {Promise<any>}
   */
  protected async activateEntity(
   ...
   * @returns {Promise<any>}
   */
  protected async disableEntityByStatus(
    entity: Entity | Entity[],
    user: IUserOrBusinessReq,
  ): Promise<any> {
  ...
  /**
   * Groups array items by the value of a string-keyed property.
   *
   * @param array - Objects to group (must allow string index access).
   * @param key - Property name used as the group key.
   * @returns Record mapping serialized key values to item arrays.
   */
  protected groupBy = <T extends Record<string, unknown>>(
    array: T[],
    key: string,
  ): Record<string, T[]> => {
  ...
  protected async orderBy(
    data: any[],
    // eslint-disable-next-line @typescript-eslint/no-unsafe-function-type -- lodash orderBy iteratee
    ...
    if (query instanceof Array) {
      ...
      if (options.orderBy) {
        query = _.orderBy(
          query,
          [options.orderBy],
          [options.order && options.order === OrderEnum.DESC ? 'desc' : 'asc'],
        );
      }
   ...
   */
  protected paginateForInfiniteScroll(
    entity: any[],
    page: number,
  ): IInfiniteScroll {
    return {
  ...
  protected async updateEntity(
    ...
    return this.cleanObjects(
      await this.repository.findOneOrFail({ where: { id: entity.id } as any }),
    );
  }
