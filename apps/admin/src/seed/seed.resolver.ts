...
export class SeedResolver {
  ...
  async seedDevelopmentBusinesses(): Promise<boolean> {
    for (const item of SEED_BUSINESSES)
      await this.seedService.seedOneBusiness(item);
    return true;
  }
  ...
  async seedDevelopmentCatalogs(): Promise<boolean> {
    for (const item of SEED_CATALOGS)
      await this.seedService.seedOneCatalog(item);
    return true;
  }
  ...
  async seedDevelopmentProducts(): Promise<boolean> {
    for (const item of SEED_PRODUCTS)
      await this.seedService.seedOneProduct(item);
    return true;
  }
}
